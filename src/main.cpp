...
int main()
{
    ...
    {
        vector<LevelData> allLevels = loadAllLevelsFromFile("all_levels.json");
        int selected = 1; // Change this to pick another level
        if (!allLevels.empty())
        {
            Game game(screenWidth, screenHeight, allLevels[selected]);
        }
        else
        {
